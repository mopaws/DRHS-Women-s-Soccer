...
def main():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('SELECT id, team, playstyle, corners, players_to_watch, goalkeeper, team_notes, roster, date_added, mimetype FROM scouting_report ORDER BY team,date_added DESC;')
    data = cur.fetchall()
    cur.close()
    conn.close()
